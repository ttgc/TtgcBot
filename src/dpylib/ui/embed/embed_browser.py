...
class EmbedBrowserView(View):
    def __init__(self, embed: DiscordEmbedMeta, *, timeout: Optional[float] = None, owner: Optional[UserType] = None) -> None:
    ...
    async def get_initial_page(self, ctx: 'ExtendedContext', *, page: int = 0, localize: bool = True) -> discord.Embed:
        self.ctx = ctx
        self.cur_page = page if page <= self.page_count else 0
        self._reset_buttons()
        ...
        self._goto_modal = await self._generate_modal()
        return self._build_page().convert(policy=EmbedConversionPolicy.TRUNCATE)
