...
class Other(commands.Cog):
    ...
    async def setlang(self, ctx, lg):
        ...
        if lang_exist(lg):
            oldlg = await DBMember.getuserlang(ctx.message.author.id)
            await DBMember.setuserlang(ctx.message.author.id, lg)
            await ctx.message.channel.send(get_lang(lg)["setlang"].format(lg))
            self.logger.info("user %d switched lang from %s to %s", ctx.message.author.id, oldlg, lg)
        else:
            data = await GenericCommandParameters(ctx)
